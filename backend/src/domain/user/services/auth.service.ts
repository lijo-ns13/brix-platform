...
import { SignUpResponseDTO,SignInResponseDTO } from "../dtos/user.response.dto";
...
export class UserService {
  ...
  async signUp(payload: SignupRequestDTO): Promise<SignUpResponseDTO> {
    ...
    const existingTempUser=await this.tempuserRepositary.findByEmail(payload.email);
    if (existingUser) {
  ...
  async signIn(payload: SigninRequestDTO): Promise<SignInResponseDTO> {
    ...
    // token generation
    const userAccessToken=JWTService.generateAccessToken('user',{id:user._id.toString(),email:user.email})
    const userRefreshToken=JWTService.generateRefreshToken('user',{id:user._id.toString(),email:user.email})
  ...
  async verifyOTP(email:string,otp:string) {
    ...
    const otpRecord=await this.otpRepositary.findOTPByAccount(tempUser._id,'user');
